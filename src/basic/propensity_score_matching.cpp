...
double Propensity_Score_Matching::solve(int influence_neighbours)
{
    ...
    {
        double min_dis = abs(possibilities[i] - possibilities[ pair_node[i] ]);
        if(min_dis * min_dis <= 4 * sqr_deviation )
        {
    ...
    sort(untreated_nodes.begin(),untreated_nodes.end());
    auto endit = unique(untreated_nodes.begin(), untreated_nodes.end());
    untreated_nodes.resize(untreated_nodes.begin(), endit); */
